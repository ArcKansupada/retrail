...
**Working name:** `retrail` (rename later — "replay" + "trail")
...
| **Step SHA** | A short content hash identifying a specific step's exact state (input messages + output), used to address it directly — e.g. `retrail fork a1b2c3d` instead of `retrail fork <session-id> --from-step 4`. |
...
SQLite, single local file (`.retrail/sessions.db`), tree-structured from the start:
...
`sha` is indexed and unique — computed as a short hash of `(session_id, step_number, input_json, output_json)` at write time. Supports prefix matching for CLI convenience (`retrail fork a1b2` resolving to the full SHA), same UX as git's short hashes.
...
5. The new session's first re-executed step gets its own SHA; the forked-from step's SHA is stored as `parent_sha` for provenance, so `retrail log` can show exactly which recorded step a fork branched from, permanently, even after the original session is deleted or renamed.
...
```
retrail bisect <session-id> --check "output contains 'confirmed'"
```
...
```
retrail init                                  # creates .retrail/ + sqlite db
retrail list                                  # sessions, tree view
retrail log <session-id>                      # step-by-step history, SHA per step
retrail show <sha>                            # full detail on one step, by SHA
retrail fork <sha> --edit-file edit.json      # fork from a step SHA, not session+step
retrail diff <session-a> <session-b>
retrail bisect <session-id> --check "<condition>"   # which step doomed a failed run
retrail ablate <session-id> --check "<condition>"   # which facts a good run needed (6.7)
retrail sweep <sha> --values-file values.json      # find a threshold (6.7)
```

SHA prefix matching applies throughout, same convention as git (`retrail show a1b2` resolves uniquely as long as the prefix is unambiguous).
...
**Status:** all six milestones are implemented (`prototype/`, `retrail/`, `tests/`, `examples/`). 122 tests pass against a scripted model; live-API validation is the outstanding gap (see Section 11).
...
1. ✅ **Recording works.** Decorator logs a real raw-loop run to SQLite (with SHA per step); `retrail log` prints it. Demoable on its own.
...
- [x] ~~Package/project name~~ — **resolved: `retrail` is unclaimed on PyPI** (checked 2026-07-16; `retrace` is taken).
...
- PyPI (`pip install retrail`), matches existing publishing plan for `chunklet`.
